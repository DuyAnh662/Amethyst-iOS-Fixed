...
void init_settings() {
    ...
    // iOS: Read settings from environment variables first (set by launcher),
    // fall back to optimized defaults for iPhone 8 Plus / A11 GPU

    int envAngle = 0;
    GetEnvVarInt("MG_ANGLE", &envAngle, 0);
    global_settings.angle = (envAngle > 0) ? AngleMode::Enabled : AngleMode::Disabled;

    int envNoError = 0;
    GetEnvVarInt("MG_NOERROR", &envNoError, 1);
    switch (envNoError) {
        case 2: global_settings.ignore_error = IgnoreErrorLevel::Full; break;
        case 1: global_settings.ignore_error = IgnoreErrorLevel::Partial; break;
    ...
    int envGLSLSize = 0;
    GetEnvVarInt("MG_GLSL_CACHE_SIZE", &envGLSLSize, 30);
    global_settings.max_glsl_cache_size = (envGLSLSize > 0) ? envGLSLSize * 1024 * 1024 : 0;
    ...
    int envGLVersion = 0;
    GetEnvVarInt("MG_GL_VERSION", &envGLVersion, 40);
